...
1. Datasets below are required for these analyses, seperated by ocean: Atlantic, Pacific:
...
three_lat_trends_atl<-ggplot(data=atl_lat_all3,aes(mid,area,colour=type))+
geom_point()+
...
#  facet_grid(.~type)
three_lat_trends_atl

ggsave(filename="three_lat_trends_atl.tiff",path="figs/")
...
pred_lat_plot_atl<-ggplot(data=cbind(atl_lat_all3,pred),
       aes(mid,area,color=type))+geom_point()+
  facet_grid(.~type)+geom_line(aes(y=pred))
pred_lat_plot_atl

pred_lat_plot_atl2<-ggplot(data=atl_lat_all3,
       aes(mid,area,color=type))+geom_point()+
  facet_grid(.~type)+geom_smooth(aes(y=area))
pred_lat_plot_atl2

ggsave(filename="pred_lat_plot_atl.tiff",path="figs/")
...
plotting all lat on one plot for pac
```{r}
install.packages("tidyverse")
install.packages("plotly")
install.packages("cowplot")
...
#log the dependent variable:
pac_lat_all3<-pac_lat_all %>% mutate(area=log10(area))

write.csv(pac_lat_all3,"output/pac_lat_all3.csv")
...
ggsave(filename="three_lat_trends_pac.tiff",path="figs/")
...
ggsave(filename="pred_lat_plott_pac.tiff",path="figs/")


```
mod1: area size is the dependent,  mid lat is the covariate, with type (1d vs 2d and 2d planar) as the factor. 
Summary of the results show mid lat does not have an effect on area but the area is dependent on the type (not suprising because the types have different dimensions of areas included. however comparing the models with anova, the interaction between area and type are not significant, with p value of .96, so area size will depend in a similar manner on mid latitude of species for any range construction type!

last plot is of predicted values from model2


plotting all long on one plot for pac
```{r}
...
#log the dependent variable:
pac_long_all3<-pac_long_all %>% mutate(area=log10(area))

write.csv(pac_long_all3,"output/pac_long_all3.csv")

three_long_trends_pac<-ggplot(data=pac_long_all3,aes(mid_long_adjust,area,colour=type))+
geom_point()+
geom_smooth(se=F,method=loess)+
#scale_y_continuous(trans='log2')+
xlab("mid longitude (degrees longitude)")+
ylab("range area (m/m2)")
three_long_trends_pac

ggsave(filename="three_long_trends_pac.tiff",path="figs/")

mod1<-aov(area~mid_long_adjust*type,data=pac_long_all3)
summary (mod1)
gam

mod2<-aov(area~mid_long_adjust+type,data=pac_long_all3)
summary (mod2)
...
pred_long_plot_pac<-ggplot(data=cbind(pac_long_all3,pred),
       aes(mid_long_adjust,area,color=type))+geom_point()+
  facet_grid(.~type)+geom_line(aes(y=pred))
pred_long_plot_pac

ggsave(filename="pred_long_plott_pac.tiff",path="figs/")

```
mod1: area size is the dependent,  mid long is the covariate (used adjusted mid long value "mid_long_adjust" to account for degree shifts across meridian), with type (1d vs 2d and 2d planar) as the factor. 
Summary of the results show mid lat does not have an effect on area but the area is dependent on the type (not suprising because the types have different dimensions of areas included. however comparing the models with anova, the interaction between area and type are not significant, with p value of .96, so area size will depend in a similar manner on mid latitude of species for any range construction type!

last plot is of predicted values from model2

plotting all long on one plot for atl
```{r}
...
#log the dependent variable:
atl_long_all3<-atl_long_all %>% mutate(area=log10(area))

write.csv(atl_long_all3,"output/atl_long_all3.csv")

three_long_trends_atl<-ggplot(data=atl_long_all3,aes(mid,area,colour=type))+
geom_point()+
geom_smooth(se=F,method=loess)+
#scale_y_continuous(trans='log2')+
xlab("mid longitude (degrees latitude)")+
ylab("range area (m/m2)")
three_long_trends_atl

ggsave(filename="three_long_trends_atl.tiff",path="figs/")

mod1<-aov(area~mid*type,data=atl_long_all3)
summary (mod1)

mod2<-aov(area~mid+type,data=atl_long_all3)
summary (mod2)
...
pred_long_plot_atl<-ggplot(data=cbind(atl_long_all3,pred),
       aes(mid,area,color=type))+geom_point()+
  facet_grid(.~type)+geom_line(aes(y=pred))
pred_long_plot_atl

ggsave(filename="pred_long_plot_atl.tiff",path="figs/")
```
mod1: area size is the dependent,  mid long is the covariate, with type (1d vs 2d and 2d planar) as the factor. 
Summary of the results show mid lat and type have significant effects on area, with interaction between two variables not significant.
mod2: Summary of the results show mid lat and type have significant effects on area, with interaction between two variables not significant.

comparing two models  anova, interaction of the variables is insignificant p=.6. area size will depend in a similar manner on mid latitude of species for any range construction type! worth noting that the p value is clsoe to .05. seems type long extent is the most different trend out of the three. 

last plot is of predicted values from model2

plotting all depth on one plot for atl
```{r}
...
#log the dependent variable:
atl_depth_all3<-atl_depth_all %>% mutate(area=log10(area))

write.csv(atl_depth_all3,"output/atl_depth_all3.csv")

two_depth_trends_atl<-ggplot(data=atl_depth_all3,aes(mid,area,colour=type))+
geom_point()+
geom_smooth(se=F,method=loess)+
#scale_y_continuous(trans='log2')+
xlab("mid depth (m)")+
ylab("range area (m/m2)")
two_depth_trends_atl

ggsave(filename="two_depth_trends_atl.tiff",path="figs/")

mod1<-aov(area~mid*type,data=atl_depth_all3)
summary (mod1)

mod2<-aov(area~mid+type,data=atl_depth_all3)
summary (mod2)
...
pred_depth_plot_atl<-ggplot(data=cbind(atl_depth_all3,pred),
       aes(mid,area,color=type))+geom_point()+
  facet_grid(.~type)+geom_line(aes(y=pred))
pred_depth_plot_atl

ggsave(filename="pred_depth_plot_atl.tiff",path="figs/")

```
mod1: area size is the dependent,  mid depth is the covariate, with type (1d vs 2d and 2d planar) as the factor. 
Summary of the results show mid depth and type have significant effects on area, with interaction between two variables not significant.
mod2: Summary of the results show mid depth and type have significant effects on area, with interaction between two variables not significant.

comparing two models  anova, interaction of the variables is insignificant p=.6. area size will depend in a similar manner on mid depth of species for any range construction type 

last plot is of predicted values from model2


plotting all depth on one plot for pac
```{r}
...
#log the dependent variable:
pac_depth_all3<-pac_depth_all %>% mutate(area=log10(area))

write.csv(atl_depth_all3,"output/pac_depth_all3.csv")

two_depth_trends_pac<-ggplot(data=pac_depth_all3,aes(mid,area,colour=type))+
geom_point()+
geom_smooth(se=F,method=loess)+
#scale_y_continuous(trans='log2')+
xlab("mid depth (m)")+
ylab("range area (m/m2)")
two_depth_trends_pac

ggsave(filename="two_depth_trends_pac.tiff",path="figs/")

mod1<-aov(area~mid*type,data=pac_depth_all3)
summary (mod1)

mod2<-aov(area~mid+type,data=pac_depth_all3)
summary (mod2)
...
pred_depth_plot_pac<-ggplot(data=cbind(pac_depth_all3,pred),
       aes(mid,area,color=type))+geom_point()+
  facet_grid(.~type)+geom_line(aes(y=pred))
pred_depth_plot_pac

ggsave(filename="pred_depth_plot_pac.tiff",path="figs/")
...
#ggplot(atl_depth_rich3,aes(depth,rich))+
#geom_line(aes(y=type$richness2d,color="2d"),size=1)+ 
#geom_line(aes(y=type$richness1d,color="1d"),size=1)+
#xlab(label="Depth (m)")+
#ylab("Richness")+
#theme_classic()

#to convert dataset from wide to long:
atl_depth_rich2<- melt(atl_depth_rich, id.vars=c("X1","depth"))
atl_depth_rich2

#to save output
write.csv(atl_depth_rich2,"output/atl_depth_rich2.csv")
...
mod1<-aov(rich~depth*type,data=atl_depth_rich3)
summary (mod1)

mod2<-aov(rich~depth+type,data=atl_depth_rich3)
summary (mod2)
...
pred_atl_depth_rich3plot<-ggplot(data=cbind(atl_depth_rich3,pred),
       aes(depth,rich,color=type))+geom_point()+
  facet_grid(.~type)+geom_line(aes(y=pred))
pred_atl_depth_rich3plot

ggsave(filename="pred_atl_depth_rich3plot.tiff",path="figs/")
...
attach(pac_depth_rich)
attach(pac_depth_rich3)
#to convert dataset from wide to long:
pac_depth_rich2<- melt(pac_depth_rich, id.vars=c("X1","depth"))
pac_depth_rich2

#to save output
write.csv(pac_depth_rich2,"output/pac_depth_rich2.csv")
...
mod1<-aov(rich~depth*type,data=pac_depth_rich3)
summary (mod1)

mod2<-aov(rich~depth+type,data=pac_depth_rich3)
summary (mod2)
...
pred_pac_depth_rich3plot<-ggplot(data=cbind(pac_depth_rich3,pred),
       aes(depth,rich,color=type))+geom_point()+
  facet_grid(.~type)+geom_line(aes(y=pred))
pred_pac_depth_rich3plot

ggsave(filename="pred_pac_depth_rich3plot.tiff",path="figs/")
...
attach(atl_lat_rich_combine3)
attach(atl_lat_rich_combine4)


#to convert dataset from wide to long:
atl_lat_rich_combine3<- melt(atl_lat_rich_combine2, id.vars=c("X1","lat"))
atl_lat_rich_combine3

#to save output
write.csv(atl_lat_rich_combine3,"output/atl_lat_rich_combine3.csv")
...
mod1<-aov(rich~lat*type,data=atl_lat_rich_combine4)
summary (mod1)

mod2<-aov(rich~lat+type,data=atl_lat_rich_combine4)
summary (mod2)
...
pred_atl_lat_rich_combine4plot<-ggplot(data=cbind(atl_lat_rich_combine4,pred),
       aes(lat,rich,color=type))+geom_point()+
  facet_grid(.~type)+geom_line(aes(y=pred))
pred_atl_lat_rich_combine4plot

ggsave(filename="pred_atl_lat_rich_combine4plot.tiff",path="figs/")
...
#to convert dataset from wide to long:
atl_long_rich_combine3<- melt(atl_long_rich_combine2, id.vars=c("X1","long"))
atl_long_rich_combine3

#to save output
write.csv(atl_long_rich_combine3,"output/atl_long_rich_combine3.csv")
...
mod1<-aov(rich~long*type,data=atl_long_rich_combine4)
summary (mod1)

mod2<-aov(rich~long+type,data=atl_long_rich_combine4)
summary (mod2)
...
pred_atl_long_rich_combine4plot<-ggplot(data=cbind(atl_long_rich_combine4,pred),
       aes(long,rich,color=type))+geom_point()+
  facet_grid(.~type)+geom_line(aes(y=pred))
pred_atl_long_rich_combine4plot

ggsave(filename="pred_atl_long_rich_combine4plot.tiff",path="figs/")
...
#to convert dataset from wide to long:
pac_temp_combine3<- melt(pac_temp_combine2, id.vars=c("X1","y"))
pac_temp_combine3

#to save output
write.csv(pac_temp_combine3,"output/pac_temp_combine3.csv")
...
mod1<-aov(rich~lat*type,data=pac_temp_combine4)
summary (mod1)

mod2<-aov(rich~lat+type,data=pac_temp_combine4)
summary (mod2)
...
pred_pac_lat_rich_combine4plot<-ggplot(data=cbind(pac_temp_combine4,pred),
       aes(lat,rich,color=type))+geom_point()+
  facet_grid(.~type)+geom_line(aes(y=pred))
pred_pac_lat_rich_combine4plot

ggsave(filename="pred_atl_lat_rich_combine4plot.tiff",path="figs/")
```


10 plot richness with long
10c. Pacific Ocean
...
#to convert dataset from wide to long:
pac_temp_combine2_long_adj2<- melt(pac_temp_combine2_long_adj, id.vars=c("X1","x","x_adjust"))
pac_temp_combine2_long_adj2

#to save output
write.csv(pac_temp_combine2_long_adj2,"output/pac_temp_combine2_long_adj2.csv")
