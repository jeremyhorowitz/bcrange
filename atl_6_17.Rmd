...
library(quantreg)
  ...
  attach(atl_ranges3)
 atl_subset<-subset(atl_ranges3,ocean=="atl")
atl_lat_latgeog<-atl_subset[ atl_subset$type %in% c("lat_range","lat_geog_range"),]

atl_lat_latgeog_nobound<-subset(atl_lat_latgeog,!(species=="bound"))

mod1<-aov(area~mid*type,data=atl_lat_latgeog)
summary(mod1)

plot(x=atl_lat_latgeog_nobound$mid,
     y=atl_lat_latgeog_nobound$area,
     col=atl_lat_latgeog_nobound$type,
     pch=16,
     xlab="mid_lat",
     ylab="range_size")


model.1=lm(area~mid+type+mid:type,data=atl_lat_latgeog_nobound)
