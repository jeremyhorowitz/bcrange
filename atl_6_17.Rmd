...
ggplot(atl_depth_rich,aes(depth))+
  geom_line(aes(y=richness1d),color="blue",size=1)+
  geom_line(aes(y=richness2d),color="red",size=1)+
  xlab(label="Depth (m)")+
  ylab("Richness")

ggplot()+
  geom_line(data=atl_rich_1d, aes(depth,richness, color="1D Bathymetric Extent"))+
  geom_line(data=species_ranges3, aes(depth,max_rich, color="2D planar Extent"))+
  xlim(0,6000)+
  ylim(1,40)
```
